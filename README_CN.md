...
- 将靠近工作区控制区域的一侧设为默认横屏方向，同时保留重力感应旋转和应用明确指定的屏幕方向。
- 退出工作区时清除全屏任务的固定尺寸，避免返回竖屏页面后仍受之前横屏区域限制。
- 改进 ColorOS 平板与大屏设备上的工作区面板、挖孔区域、系统原生浮窗和游戏助手遮挡兼容性。
- 改进视频任务在主屏幕与竖屏任务槽位之间移动时的状态恢复，包括播放器仍停留在全屏界面的情况。
...
NeXtep 目前仍是一个**处于早期阶段、针对特定设备开发的项目**。0.3.0 版本根据 OPPO Pad / ColorOS 16 的用户反馈加入了兼容性调整，但对于未经测试的系统版本或设备，项目不作兼容性保证。开发过程中使用的本地录屏、提取的厂商应用、日志和设备转储文件均不会包含在仓库中。
