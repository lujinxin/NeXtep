...
- **适配视频应用横屏播放**，横屏期间保留控制区域和三个任务槽位，并在退出横屏时按当前设备显示尺寸恢复主任务。
...
NeXtep 目前仍是一个**处于早期阶段、针对特定设备开发的项目**。对于未经测试的 ColorOS 版本或设备，项目不作兼容性保证。开发过程中使用的本地录屏、提取的厂商应用、日志和设备转储文件均不会包含在仓库中。
