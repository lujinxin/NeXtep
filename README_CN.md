...
- 新增应用内版本检查，方便用户及时获取 NeXtep 新版本。
- 在设置应用中显示当前版本、可用版本和版本更新说明。
- 发现新版本后，可跳转浏览器打开下载页面。
...
NeXtep 目前仍是一个**处于早期阶段、针对特定设备开发的项目**。0.3.1 版本加入了应用内更新检查与浏览器 APK 下载流程，不会自动安装更新。对于未经测试的系统版本或设备，项目不作兼容性保证。开发过程中使用的本地录屏、提取的厂商应用、日志和设备转储文件均不会包含在仓库中。
